...
let configFile = 'ss-config.json', dbFile = 'ss-up.enc', syncFile = 'sync-info.json', uuidFile = 'uuid', errorLogFile = 'error.log',
    userPath, configFilePath, DBFilePath, syncFilePath, errorLogFilePath, uuidFilePath, ssConfig = null, mysqlParams = null, syncInfo = null,
...
app.on('ready',() => {
    ...
    uuidFilePath = path.join(userPath,uuidFile); syncFilePath = path.join(userPath,syncFile); errorLogFilePath = path.join(userPath,errorLogFile);
...
ipcMain.on('get-all-config',function(event){ event.returnValue = { ssConfig,mysqlParams,syncInfo,syncFilePath,userPath,uuidFilePath,errorLogFilePath } });
